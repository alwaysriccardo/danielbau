...
const PromoPopup: React.FC<PromoPopupProps> = ({ onClose }) => {
  ...
  return (
    ...
    >
      <div 
        className={`bg-[#E3E1DC] border-2 border-[#121212] rounded-lg shadow-[0_8px_32px_rgba(0,0,0,0.3)] max-w-md w-[90%] max-h-[90vh] overflow-y-auto transition-all duration-300 ${
          isVisible ? 'scale-100 opacity-100' : 'scale-95 opacity-0'
        }`}
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="p-6 pb-4 border-b border-[#121212]/20">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <span className="text-2xl font-bold text-[#121212]">10%</span>
              <span className="text-xs font-semibold text-[#121212]/70 uppercase tracking-wider">
                {t.promo.off}
              </span>
            </div>
            <button
          ...
          <h2 className="text-lg font-semibold text-[#121212] mb-1">
            {t.promo.title}
          </h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            <input
              ...
              required
              className="w-full px-4 py-2.5 border border-[#121212]/30 rounded bg-white/50 focus:outline-none focus:border-[#121212] focus:bg-white transition-colors text-[#121212] placeholder:text-[#121212]/50"
            />
          </div>
          ...
          <div>
            <select
              ...
              onChange={handleChange}
              className="w-full px-4 py-2.5 border border-[#121212]/30 rounded bg-white/50 focus:outline-none focus:border-[#121212] focus:bg-white transition-colors text-[#121212]"
            >
          ...
          <div>
            <textarea
              ...
              rows={3}
              className="w-full px-4 py-2.5 border border-[#121212]/30 rounded bg-white/50 focus:outline-none focus:border-[#121212] focus:bg-white transition-colors text-[#121212] placeholder:text-[#121212]/50 resize-none"
            />
          </div>
          ...
          <div className="flex flex-col gap-2 pt-2">
            <button
              ...
              className="w-full px-6 py-3 bg-[#121212] text-[#E3E1DC] font-semibold rounded hover:bg-[#121212]/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
            ...
            <button
              type="button"
              onClick={onClose}
              className="w-full px-6 py-2.5 text-[#121212]/70 hover:text-[#121212] transition-colors text-sm"
            >
              {t.promo.noThanks}
            </button>
          </div>
